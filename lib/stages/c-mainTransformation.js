...
function transformNode(node) {
    ...
    if (node.type === 'CallExpression' && node.callee.type === 'MemberExpression' && /^await/g.test(node.source())) {
        node.update(node.source().replace(/^await/g, ''));//Remove excess await
    }
    if (node.type === 'ReturnStatement' && node.argument.type === 'MemberExpression') {
        node.update(node.source().replace(/^return await/g, 'return '));
    }
    if (/await_awaitResults/.test(node.source())) {
        node.update(node.source().replace(/await_awaitResults/g, '_awaitResults'));
        //console.warn('We may have done something wrong with ' + node.type);
    }
