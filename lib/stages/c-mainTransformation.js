...
statementTransformers['WhileStatement'] = function transformWhileStatement(node) {
    console.log(node);
        node.update(secretPrefix + 'currentStep = ' + stepID + ';\nbreak;\ncase ' + stepID + ':' + 
            'if(' + node.test.source() + ') {' + secretPrefix + 'currentStep = ' + (stepID + 1) + '} else {' + secretPrefix + 'currentStep = ' + (stepID + 2) + '}\nbreak;' + 
            '\ncase ' + (stepID + 1) + ':' + removeBraces(node.body.source()) + secretPrefix + 'currentStep = ' + (stepID) + '\nbreak;' +
            '\ncase ' + (stepID + 2) + ':');
};
...
function transformNode(node) {
    ...
    if (node.type === 'CallExpression' && node.callee && node.callee.type === 'Identifier' && node.callee.name === 'await') {
        ...
        node.body.update(template.innerStepFunction({
            vars: 'random' ||node.varsToDeclare.join(', '),
