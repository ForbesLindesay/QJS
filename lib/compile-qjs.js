...
compile(module, function () {
    module.exports.run = function (foo, bar) {
        if (bar) {
            return await(foo());
        } else {
            return await(foo);
...
//debug(module.exports.run);
//
module.exports.m = function m() {
    return Q.reject(new Error('a'));
}

compile(module, function () {
    function go() {
        await(Q.delay('a', 1000));
        console.log('result A');
        console.log(await(module.exports.run(module.exports.m, true)));
        console.log('result B');
        console.log(await(module.exports.run(Q.delay('b', 500), true)));
    }
    go().end();
});
